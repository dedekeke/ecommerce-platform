...
describe('httpErrorInterceptor', () => {
  ...
  it('should toast the server-provided message field on error', (done) => {
    http.get('/api/products').subscribe({
      error: () => {
        expect(toastSpy.error).toHaveBeenCalledWith('SKU already exists');
        done();
      },
    });

    const req = httpMock.expectOne('/api/products');
    req.flush({ message: 'SKU already exists' }, { status: 409, statusText: 'Conflict' });
  });
  ...
  it('should toast the server-provided error field when message is absent', (done) => {
    ...
    const req = httpMock.expectOne('/api/products');
    req.flush({ error: 'Category not found' }, { status: 404, statusText: 'Not Found' });
  });
