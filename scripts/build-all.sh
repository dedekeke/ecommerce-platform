...
if [ "$DEPLOY" = true ]; then
    ...
    echo ""
    echo "Microservices (docker-compose published ports):"
    ...
    echo "  Search Service:    http://localhost:8088/swagger-ui.html"
    echo "  Media Service:     http://localhost:8089/swagger-ui.html"
    echo "  Promotion Service: http://localhost:8090/swagger-ui.html"
    echo ""
    echo "Not in the local compose stack (run via Maven instead):"
    echo "  notification-service, recommendation-service, review-service"
    echo ""
    echo "Monitoring:"
    echo "  Zipkin:            http://localhost:9411"
    echo "  Grafana:           http://localhost:3000 (admin/admin, docker-compose.monitoring.yml)"
    echo "  Prometheus:        http://localhost:9090 (docker-compose.monitoring.yml)"
    echo ""
    echo -e "${BLUE}============================================${NC}"
    echo ""
    echo "To check services:   ./scripts/check-services.sh --docker"
