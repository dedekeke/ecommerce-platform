...
class OutboxRelayTest {
    ...
    void should_publishAndMarkAllEvents_when_batchSucceeds() {
        List<OutboxEvent> batch = List.of(
            buildEvent(1L, "order-1", "order.created"),
        ...
        assertThat(sent).extracting(ProducerRecord::topic)
            .containsExactly("order.created", "order.updated", "order.cancelled");
    ...
    void should_skipMarkingFailedRow_when_kafkaSendThrows() {
        OutboxEvent ok1 = buildEvent(1L, "order-ok", "order.created");
    ...
    private OutboxEvent buildEvent(Long id, String aggregateId, String topic) {
