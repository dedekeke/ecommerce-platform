...
- Name: golden-path.spec.ts >> Golden path: product list → cart → checkout → confirmation >> homepage renders welcome heading and category links
- Location: tests/golden-path.spec.ts:20:3
...
Locator: getByRole('heading', { name: /welcome to e-commerce/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: /welcome to e-commerce/i })
