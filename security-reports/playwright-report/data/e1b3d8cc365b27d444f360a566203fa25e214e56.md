...
- Name: golden-path.spec.ts >> Golden path: product list → cart → checkout → confirmation >> navigating to /products renders the product list page
- Location: tests/golden-path.spec.ts:28:3
...
Locator: getByRole('heading', { name: /all products/i })
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: /all products/i })
